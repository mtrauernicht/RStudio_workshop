...
# Gender distribution per course at JNEC in the pre-workshop questionnaire
## Tip: use the filter function from dplyr
ggplot(pre_workshop_data_filt %>% 
         filter(College == "JNEC"),
       aes(x = Gender)) +
  geom_bar() +
  facet_wrap(~Course) +
  labs(title = "Gender distribution per course at JNEC in the pre-workshop questionnaire") +
  theme_bw()
```
Conclusion: At JNEC, mostly female students participated from Diploma in Construction Supervision.
...
ggplot(pre_workshop_data_filt,
       aes(x = Importance)) +
  geom_bar() +
  labs(title = "Importance rating of cybersecurity in the pre-workshop questionnaire") +
  theme_bw() +
  facet_wrap(~College)

# Gender distribution per course at JNEC in the post-workshop questionnaire
ggplot(post_workshop_data_filt,
       aes(x = Importance)) +
  geom_bar() +
  labs(title = "Importance rating of cybersecurity in the post-workshop questionnaire") +
  theme_bw() +
  facet_wrap(~College)
```
Conclusion: At all colleges, the importance ranking increased after the workshop.
...
# Correlation test between awareness and importance ratings in the pre-workshop questionnaire
cor.test(pre_workshop_data_filt$Awareness, pre_workshop_data_filt$Importance, method = "spearman")
```
Conclusion: There is no correlation between pre-workshop awareness and importance ratings.
...
# Plot importance ratings by Gender
ggplot(pre_workshop_data_filt,
       aes(x = Importance)) +
  geom_bar() +
  labs(title = "Importance rating of cybersecurity by Gender in the pre-workshop questionnaire") +
  theme_bw() +
  facet_wrap(~Gender)

# Mann-Whitney U test to compare importance
wilcox.test(pre_workshop_data_filt$Importance ~ pre_workshop_data_filt$Gender)
```
Conclusion: There is no significant difference in importance ratings between male and female.
...
# Wilcoxon signed-rank test to compare importance ratings before and after the workshop
wilcox.test(post_workshop_data_filt$Importance, pre_workshop_data_filt$Importance, alternative = "greater")
```
Conclusion: Importance ratings significantly increased after the workshop.
